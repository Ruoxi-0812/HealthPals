...
const REAL_FALLBACK_COVERS = [
  "img/articles/heart-healthy-real.jpg",
  "img/articles/diet-cells.jpg",
  "img/articles/screen-sleep.jpg",
  "img/articles/hydration.jpg",
  "img/articles/allergies.jpg",
  "img/articles/principles-health.jpg",
  "img/articles/health-indicators.jpg",
  "img/articles/heart-healthy.jpg",
];
...
function fallbackRealCoverSrc(seedInput) {
  const seed = hashString(seedInput || "article");
  const path = REAL_FALLBACK_COVERS[seed % REAL_FALLBACK_COVERS.length];
  return toAbsoluteAsset(path);
}
...
export function newsCoverSrc(cover, newsId) {
  ...
  if (cover == null || typeof cover !== "string") {
    return fallbackRealCoverSrc(newsId);
  }
  const trimmed = cover.trim();
  if (!trimmed) return fallbackRealCoverSrc(newsId);
  ...
  if (!trimmed.startsWith("http")) {
    return fallbackRealCoverSrc(newsId || trimmed);
  }
  return trimmed;
}
...
export function onCoverImgError(event) {
  ...
  el.src = fallbackRealCoverSrc(el.dataset.newsId || el.alt || el.src);
}
