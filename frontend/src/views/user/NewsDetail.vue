<template>
  <el-row style="width: 1000px; margin: 0 auto">
        ...
        <div>
          ...
          >
            ...
            <el-button
              ...
              class="customer"
              size="mini"
              >{{ !saveFlag ? "Save Now" : "Unsave" }}</el-button
            >
          </div>
    ...
    <el-col :span="8">
      <h3 style="padding: 10px 30px">Recommended News</h3>
      <el-col
        @click.native="newsItemClick(news)"
        :span="24"
        :key="index"
...
export default {
  ...
  created() {
    // First: Retrieve stored news data
    this.getStorageInfo();
    this.loadAllTopNews();
  },
  methods: {
    loadSaveStatus() {
      ...
      console.log("Query parameters：" + JSON.stringify(newsSaveQueryDto));
      this.$axios
